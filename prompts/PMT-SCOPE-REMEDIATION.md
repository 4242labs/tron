...
Author it all on a branch and report the branch name (`--tag branch --branch <name>`); I gate it, then you land it on trunk yourself via the grant + `meta/scripts/land.sh` (I never land or merge — that's yours). Report the adhoc blocks you authored and any you adjusted, or "no valid findings."

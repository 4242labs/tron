...
def _archive_path(block_file):
    """The §6 close-out archival destination of a block doc: `blocks/<id>.md`
    -> `blocks/archive/<id>.md` (an `archive/` dir inserted before the basename,
    within the doc's own directory). The frozen session-end skill's `git mv`
    target."""
...
def record_commit_ok(repo_root, block_file, dry=False, truth_ref="main",
    ...
    archive_path = _archive_path(block_file)
    ...
    # are the same archival; anything else is out-of-gate.
    archived = False
    ...
    if pipeline_file and block_id:
        ...
        for s in pl_lines:
            if s and block_id not in s:
                return False, (f"record commit {sha[:8]} edits a pipeline.md line not "
                               f"naming block {block_id!r} — out-of-lane (§6: your own row only)")
    return True, sha[:8]

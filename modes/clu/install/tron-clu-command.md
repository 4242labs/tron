...
1. Your root is `CLU_ROOT=<CLU_ROOT>` — the CLU mode directory, written into this command file at install time (`install/README.md`). If that literal is still `<CLU_ROOT>` or the path has no `clu.md`, the install is incomplete: tell the operator to re-run the install step, and stop. Every CLU file below lives under that root.

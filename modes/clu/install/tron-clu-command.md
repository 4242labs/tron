...
1. Resolve the CLU root — CLU mode directory on this machine (`<tron-app>/modes/clu`): `CLU_ROOT=$(cat ~/.claude/tron-clu.path)`. If the pointer file is missing or the path it names has no `clu.md`, ask the operator for the path and write it: `echo "<tron-app>/modes/clu" > ~/.claude/tron-clu.path`. Every CLU file below lives under that root.
